...
function App() {
  ...
  const {
    ...
    canadaStates, usStates, deleteAddressPopup, selectedDeletedAddress, loginError, isForgetPassword, forgetPasswordEmail} = state
      ...
      if (addresses.length <= 0) {
        ...
        if (cookies.get(custemail)) {
          createGuestUser({ email: cookies.get(custemail), sessionid: cookies.get(PU_UserID) }).then(res => {
            ...
            editOrNewGuestAddress({ ...value, CUSTOMERID: res.Result })
          ...
          <Alert show={true} variant="secondary" className={'alertCustm'}>
            ...
            <Row>
              ...
              <Col xs={6} className='pl-1'>
                <Button
                  onClick={() => { forgetPassword(forgetPasswordEmail); setState({...state, isForgetPassword: false}); alert('password will be sent to your email address') }}
        ...
        <Container>
          ...
          <Row style={{ minHeight: 50, }} className='justify-content-center justify-content-sm-start'>
            ...
            <Col xs={12} sm={6} md={4} lg={3} style={{ display: 'flex', justifyContent: 'left', alignItems: 'center' }} className='justify-content-center justify-content-sm-start ml-auto'>
              ...
              <img src="./img/logo.png" className="brand-logo mb-2 mb-sm-0" style={{ height: 30 }} />
            </Col>
